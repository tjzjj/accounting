<template>
  <q-page class="fullscreen flex flex-center">
    ...
    <q-card class="q-pa-lg login-card">
      ...
      <div class="text-center text-italic text-black-7 text-bold"> Registry of Appropriation and Commitment System (RACS)</div>
      ...
      <q-form @submit="onSubmit" class="q-gutter-md">
        ...
        <q-btn type="submit" label="Sign In" class="custom-btn" />
        </div>
        <div class="text-center q-mb-md">
        <span class="text-grey-7">Switch to </span>
        <a href="#" class="text-primary text-bold" @click="switchToAdmin">User</a>
      </div>
      </q-form>
    </q-card>
  </q-page>
...
export default {
  ...
  methods: {
    ...
    switchToAdmin() {
      // Simulated admin switch functionality (can be replaced with logic)
...
.login-card {
  ...
  max-width: 400px;
  border-radius: 15px;
  background: rgba(255, 255, 255, 0.811); /* Semi-transparent white */
  padding: 20px;
  border: 3px solid rgba(0, 128, 0, 0.7); /* Green Border */
...
.custom-btn {
  ...
  max-width: 200px; /* Ensure it doesn't stretch */
  text-align: center;
  cursor: pointer;
  display: block;
